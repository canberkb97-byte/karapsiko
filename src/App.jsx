...
const Disclaimer = ({ onAccept }) => (
  <div style={DS.overlay}>
    <div style={DS.box}>
      <FaceLogo size={40} />
      <h2 style={DS.title}>KaraPsiko — Gölge Danışman</h2>
      <div style={DS.text}>
        <p>Bu uygulama bir <strong>psikolojik danışmanlık, terapi veya tıbbi destek hizmeti değildir.</strong></p>
        <p>KaraPsiko yalnızca iletişim stratejileri, müzakere teknikleri ve sosyal dinamikler hakkında bilgi ve öneri sunmak amacıyla tasarlanmıştır.</p>
        <p>Psikolojik sorunlar yaşıyorsanız lütfen bir ruh sağlığı uzmanına başvurunuz.</p>
        <p style={{ color: "#555", fontSize: 11, marginTop: 12 }}>Devam ederek bu koşulları kabul etmiş olursunuz.</p>
      </div>
...
const DS = {
  ...
  box: { background: "#111", border: "1px solid #222", borderRadius: 16, padding: "28px 24px", maxWidth: 400, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 },
  title: { margin: 0, fontSize: 16, fontWeight: 700, color: "#e8e4e0", letterSpacing: 2 },
  text: { fontSize: 13, lineHeight: 1.7, color: "#999", textAlign: "center" },
  btn: { marginTop: 8, padding: "13px 40px", background: "#c0392b", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, letterSpacing: 1, cursor: "pointer", width: "100%" },
};
...
export default function App() {
  ...
  useEffect(() => {
    const accepted = localStorage.getItem("kp_disclaimer");
    if (accepted === "true") setDisclaimerAccepted(true);
  }, []);

  const acceptDisclaimer = () => {
    setDisclaimerAccepted(true);
    localStorage.setItem("kp_disclaimer", "true");
  };
...
const S = {
  ...
  card:{background:"#0f0f0f",border:"1px solid #1a1a1a",borderRadius:10,padding:"16px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer",transition:"all .2s",textAlign:"center"},
  cn:{fontSize:12.5,fontWeight:700,color:"#e8e4e0",letterSpacing:1.5},
  cd:{fontSize:10,color:"#444"},
  ...
  tb:{display:"flex",borderBottom:"1px solid #121212",background:"#0a0a0a",overflowX:"auto"},
