...
const SocialLinks = ({ small }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: small ? 6 : 8, marginTop: small ? 12 : 8 }}>
    <div style={{ display: "flex", gap: 16 }}>
      ...
      <a href="https://www.shopier.com/karapsy/45589888" target="_blank" style={{ color: C.accent, fontSize: 11, textDecoration: "none" }}>E-Kitap</a>
    </div>
...
export default function App() {
  ...
  const deleteConvo = async (convId, e) => { e.stopPropagation(); await supabase.deleteConversation(convId, token); setConvos(prev => prev.filter(c => c.id !== convId)); if (activeConvo === convId) { setMsgs([]); setActiveConvo(null); } };
          ...
          {convos.slice(0, 5).map(conv => (
            <div key={conv.id} onClick={() => loadConvo(conv)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: C.bg2, border: "1px solid " + C.border, borderRadius: 8, marginBottom: 6, cursor: "pointer", transition: "all .2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.accent; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; }}>
              ...
              <div style={{ flex: 1, overflow: "hidden" }}>
                ...
                <p style={{ margin: "2px 0 0", fontSize: 10, color: C.textMuted }}>{MODES[conv.mode] ? MODES[conv.mode].label : conv.mode} — {new Date(conv.updated_at).toLocaleDateString("tr-TR")}</p>
              </div>
    ...
    <div style={S.root}>
      <div style={S.hdr}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <button onClick={newChat} style={S.iconBtn} title="Yeni sohbet"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg></button>
          <button onClick={() => setShowHistory(!showHistory)} style={S.iconBtn} title="Geçmiş"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={modeConvos.length > 0 ? C.accent : C.textMuted} strokeWidth="2" strokeLinecap="round"><path d="M12 8v4l3 3"/><circle cx="12" cy="12" r="9"/></svg></button>
        </div>
      </div>
      {showHistory && (
        <div style={S.histPanel}>
          <div style={S.histHeader}><span style={{ fontSize: 12, fontWeight: 700, color: C.text, letterSpacing: 1 }}>Sohbet Geçmişi</span><button onClick={() => setShowHistory(false)} style={S.iconBtn}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.textMuted} strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button></div>
          ...
          {modeConvos.map(conv => (
            <div key={conv.id} onClick={() => loadConvo(conv)} style={{ ...S.histItem, borderColor: conv.id === activeConvo ? C.accent : C.border }}>
              <div style={{ flex: 1, overflow: "hidden" }}><p style={{ margin: 0, fontSize: 12, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{conv.title || "Sohbet"}</p><p style={{ margin: "2px 0 0", fontSize: 10, color: C.textMuted }}>{new Date(conv.updated_at).toLocaleDateString("tr-TR")}</p></div>
              <button onClick={(e) => deleteConvo(conv.id, e)} style={{ ...S.iconBtn, padding: 4 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e74c3c" strokeWidth="2" strokeLinecap="round"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>
            </div>
          ))}
        </div>
      )}
...
const CSS = [
  ...
  "::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#252530;border-radius:3px}"
].join("");

const S = {
  root:{width:"100%",height:"100vh",background:C.bg,display:"flex",flexDirection:"column",fontFamily:"'DM Sans',sans-serif",color:C.text,overflow:"hidden"},
  ...
  ai:{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"12px 14px",color:C.text,fontSize:13.5,fontFamily:"'DM Sans',sans-serif",outline:"none"},
  ab:{padding:"13px",background:C.accent,color:"#000",border:"none",borderRadius:8,fontSize:13,fontWeight:700,letterSpacing:1,cursor:"pointer"},
  gb:{padding:"13px",background:C.bg3,color:C.textDim,border:"1px solid "+C.border,borderRadius:8,fontSize:13,fontWeight:500,cursor:"pointer"},
  at2:{background:"none",border:"none",color:C.textMuted,fontSize:12,cursor:"pointer",marginTop:4,padding:4},
  ...
  ue:{fontSize:11,color:C.textMuted,letterSpacing:.5},
  pb2:{fontSize:9,fontWeight:700,color:C.accent,letterSpacing:2,border:"1px solid "+C.accent,borderRadius:4,padding:"2px 6px"},
  lo:{background:"none",border:"none",color:C.textMuted,fontSize:11,cursor:"pointer"},
  brand:{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"},
  title:{margin:0,fontSize:24,fontWeight:700,letterSpacing:8,color:C.text},
  sub:{margin:"4px 0 0",fontSize:10,fontWeight:500,letterSpacing:5,color:C.accent},
  line:{width:28,height:1,background:C.border,margin:"14px 0"},
  tag:{margin:0,fontSize:13,color:C.textMuted,fontStyle:"italic",letterSpacing:.5},
  grid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,width:"100%",maxWidth:380},
  card:{background:C.bg2,border:"1px solid "+C.border,borderRadius:10,padding:"16px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer",transition:"all .2s",textAlign:"center"},
  cn:{fontSize:12.5,fontWeight:700,color:C.text,letterSpacing:1.5},
  cd:{fontSize:10,color:C.textMuted},
  usg:{width:"100%",maxWidth:380,display:"flex",flexDirection:"column",alignItems:"center",gap:6},
  ul:{fontSize:10,color:C.textMuted,letterSpacing:2,fontWeight:500},
  ut:{width:"100%",height:2,background:C.bg3,borderRadius:1,overflow:"hidden"},
  uf:{height:"100%",background:C.accent,borderRadius:1,transition:"width .4s"},
  pb:{marginTop:10,padding:"13px 30px",background:C.accent,color:"#000",border:"none",borderRadius:8,fontSize:12,fontWeight:700,letterSpacing:1.5,cursor:"pointer"},
  hdr:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",borderBottom:"1px solid "+C.border,background:C.bg},
  ...
  ht:{fontSize:12,fontWeight:700,letterSpacing:2.5,color:C.text},
  iconBtn:{background:"none",border:"none",cursor:"pointer",padding:"4px 6px",display:"flex",alignItems:"center"},
  histPanel:{background:C.bg2,borderBottom:"1px solid "+C.border,maxHeight:"50vh",overflowY:"auto",padding:"8px 12px"},
  histHeader:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8},
  histItem:{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",borderRadius:8,border:"1px solid "+C.border,marginBottom:6,cursor:"pointer",transition:"all .2s"},
  tb:{display:"flex",borderBottom:"1px solid "+C.border,background:C.bg,overflowX:"auto"},
  ...
  et:{fontSize:13,color:C.textMuted,margin:0,fontWeight:500,letterSpacing:1},
  ubub:{alignSelf:"flex-end",background:C.bg3,border:"1px solid "+C.border,borderRadius:"13px 13px 3px 13px",padding:"10px 14px",maxWidth:"82%"},
  abub:{alignSelf:"flex-start",background:"linear-gradient(160deg,#161620,#1a1a28)",border:"1px solid "+C.border,borderRadius:"13px 13px 13px 3px",padding:"12px 14px",maxWidth:"90%"},
  atag:{fontSize:8.5,fontWeight:700,color:C.accent,letterSpacing:2.5,marginBottom:6},
  mt:{fontSize:13.5,lineHeight:1.7,color:"#c8c8d8"},
  dots:{display:"flex",gap:5,padding:"3px 0"},
  cur:{color:C.accent,animation:"blink 1s infinite",fontWeight:300,fontSize:14},
  iw:{padding:"10px 12px 14px",borderTop:"1px solid "+C.border,background:C.bg},
  ir:{display:"flex",alignItems:"flex-end",gap:8,background:C.bg2,border:"1px solid "+C.border,borderRadius:11,padding:"7px 10px"},
  ta:{flex:1,background:"transparent",border:"none",outline:"none",color:C.text,fontSize:13.5,fontFamily:"'DM Sans',sans-serif",lineHeight:1.5,resize:"none",maxHeight:150,padding:"3px 0"},
  sb:{width:32,height:32,borderRadius:"50%",background:C.accent,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"opacity .2s"},
  lb:{textAlign:"center",padding:"4px 0"},
  lt:{fontSize:11.5,color:C.textMuted,margin:"0 0 8px"},
  ps:{padding:"10px 22px",background:C.accent,color:"#000",border:"none",borderRadius:8,fontSize:11.5,fontWeight:700,letterSpacing:1.5,cursor:"pointer"},
};
